...
import { saveHNPost, saveHNComments, saveFilteredPost, saveSummary } from "./db-service"
import { filterPosts, generateSummary, generateImagePrompt } from "./ai-service"
...
export async function runScraper() {
  try {
    ...
    for (let i = 0; i < filteredPosts.length; i++) {
      ...
      console.log(`Processing post ${i + 1}/${filteredPosts.length}: ${post.title}`)

      // Step 4.1: Save as a filtered post
      const { id: hnPostId } = await saveHNPost(post)
      const { id: filteredPostId } = await saveFilteredPost(hnPostId, i + 1, today)
